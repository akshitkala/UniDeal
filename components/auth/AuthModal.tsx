...
export default function AuthModal() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    const redirectTo = `${window.location.origin}/auth/callback${
      authModalOptions.returnTo ? `?next=${encodeURIComponent(authModalOptions.returnTo)}` : ''
    }`;
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4" noValidate>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-neutral-muted">or</span>
                <div className="flex-1 h-px bg-border" />
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 px-4 bg-white text-neutral-text border border-border rounded-md font-medium text-sm hover:bg-surface disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 transition-colors min-h-[44px] flex items-center justify-center gap-2.5"
              >
                {googleLoading ? (
                  <div className="w-4 h-4 border-2 border-neutral-muted/30 border-t-neutral-muted rounded-full animate-spin" />
                ) : (
                  <GoogleIcon />
                )}
