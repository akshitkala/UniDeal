...
| Companion docs | UniDeal TRD v1.1, architecture.md v1.1, design.md, appflow.md |

> This version supersedes PRD v2.0. Changes in this pass: seller identity reduced to first name only (credibility tradeoff, deliberately accepted), three new public pages added (Our Story, How It Works, Contact Us), login/signup changed from dedicated pages to a modal overlay pattern, and a contact form backed by Resend.
...
- Give sellers a lightweight visible identity (first name) so listings carry a trust signal WhatsApp groups lack, without over-exposing personal detail
...
- **Login and signup are presented as a client-side modal overlay**, not dedicated routed pages — triggered from any CTA that requires auth (Contact Seller, Sell, Dashboard, Profile). On success, the user resumes their original intent (`returnTo`) rather than landing on Home.
- `/verify-email` remains a real, dedicated route (not a modal) since it's reached via an email link, not an in-app CTA
...
- **Seller identity shown on listings is first name only** (e.g. "Akshit," not "Akshit Sharma"). This is a deliberate credibility tradeoff — a first name alone is a weaker trust signal than a full name, branch, and year, but was chosen to limit personal-detail exposure on a public listing page. Full name is still collected at signup and stored; only the first token is ever rendered to other users.
...
**Single form page** — not a multi-step modal wizard. One page, all fields, one submit. Chosen deliberately for build simplicity without sacrificing usability; a well-designed single page can feel just as polished as a wizard. Edit Listing reuses this same form, pre-filled.

## 5.7 Search & Discovery

- Full-text search across listing titles and descriptions
- Filter by category and condition
- **Sort by: Newest / Price Low→High / Price High→Low** ("Most viewed" deferred since it needs view-tracking infra not yet justified by usage data)
- Browse by category
...
- No AI quality checks (Gemini/Cloudinary moderation), no automated queue prioritization, and no full audit log in v1

## 5.9 Public Trust & Support Pages

Three additional public routes, none of which touch the core marketplace data model:

- **`/our-story`** — founder narrative and campus origin story. Credibility content, not conversion content.
- **`/how-it-works`** — short visual walkthrough of the List → Discover → Contact on WhatsApp → Meet up flow.
- **`/contact`** (nav label: **"Contact Us"**, deliberately distinct from the "Contact Seller" mechanic to avoid user confusion) — a contact form backed by the Resend API free tier. Submits to `POST /api/contact`, which sends the message via Resend. Requires a `RESEND_API_KEY` environment variable. This is a support/feedback channel for the founder, unrelated to buyer-seller contact.
...
## 6.4 Visitor / Support
- As a prospective user, I want to read the founder's story so I trust the platform is genuine, not a scam
- As a new visitor, I want a quick visual explainer of how the flow works before I sign up
- As anyone, I want a straightforward way to contact the team that isn't the seller-contact mechanic
...
| Seller identity | Only first name is ever rendered on any listing or public page; full name is never shown to other users |
| Auth modal | Login/signup opens as an overlay from any gated CTA; browser back closes the modal, not the page underneath |
| Contact form | Submitting `/contact` sends an email via Resend; failure shows a plain-language message, not a raw error |
...
| Full name / branch / year shown publicly | Deliberately reduced to first name for privacy | Reconsider only if fraud/trust data demands it |
...
- Resend free tier — sufficient for low-volume contact-form traffic at launch scale
